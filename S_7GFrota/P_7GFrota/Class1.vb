...
'				venda.Aberta = False
'				dr = cadPDV.RetornaVendasPeriodo_Status(venda.Aberta,CDate(txt_data1.Text),CDate(txt_data2.Text))
'				bs.DataSource = dr
'				dr.Close
...
''				dgv_InfoVendas.Columns(8).Width = 150
'				
'					
...
'		
'		If e.KeyCode = Keys.Enter Then
'			If dgv_InfoVendas.RowCount < 1 Then
'				Exit Sub
'			Else
'				txt_pesquisa.Text = dgv_InfoVendas.SelectedCells(0).Value.ToString
'				dgv_InfoVendas.DataSource = ""
'				txt_pesquisa.Focus
'			End If
'			
...
'				bs.DataSource = dt
'				dgv_InfoVendas.DataSource = bs	
'				dgv_InfoVendas.Columns(0).HeaderText = "PRODUTO"
'			ElseIf rb_fornecedor.Checked = True
'				dt = cadFornecedor.RetornaNomeFornecedor(txt_pesquisa.Text)
'				bs.DataSource = dt
'				dgv_InfoVendas.DataSource = bs	
'				dgv_InfoVendas.Columns(0).HeaderText = "FORNECEDOR"	
'			Else
'				Exit Sub
'			End If
'		dgv_InfoVendas.Columns(0).Width = 173
'	End Sub
'	
'	
...
'			valor = valor + CDbl(dgv_InfoVendas.item(7,linha).Value)
'			linha +=1
'		end while
'
'		Return valor
